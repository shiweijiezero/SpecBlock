...
class FutureMap:
    ...
    # SpecBlock-Shift uses a different schema than EAGLE: instead of
    # topk_p / topk_index, it future-buffers b0_logits / b0_rank_logits /
    # b0_hidden / b0_input_id (the block-0 cached outputs that the next
    # iter's draft consumes via build_tree_gpu's block-1 mega kernel).
    # Per-req lists (cross_loc, cross_count, cross_position, ttt_k/v) are
    # not future-buffered: they're populated by ``_refresh_draft_state``
    # *after* verify completes, before the next iter's draft reads them,
    # so plan-stream overlap doesn't need them in the future map.
    _SPECBLOCK_FIELDS = (
        "b0_logits", "b0_rank_logits", "b0_hidden", "b0_input_id",
        "verified_id", "new_seq_lens",
    )

    def _is_specblock(self) -> bool:
        return self.spec_algo is not None and self.spec_algo.is_specblock()

    def _lazy_init_buf(self, draft_input):
        self.buf_initialized = True

        if self._is_specblock():
            for name in self._SPECBLOCK_FIELDS:
                ref = getattr(draft_input, name)
                if ref is None:
                    raise RuntimeError(
                        f"[FutureMap.SpecBlock] field {name!r} is None on "
                        f"draft_input; V2 plan_stream needs it pre-populated."
                    )
                ref0 = ref[0]
                setattr(self, f"{name}_buf", torch.empty(
                    (self.future_buffer_len, *ref0.shape),
                    dtype=ref0.dtype, device=self.device,
                ))
    ...
    def resolve_future(self, model_worker_batch: ModelWorkerBatch):
        ...
        if self._is_specblock():
            for name in self._SPECBLOCK_FIELDS:
                buf = getattr(self, f"{name}_buf")
                setattr(draft_input, name, buf[indices])
            if spec_need_hidden_states():
    ...
    def store_to_map_for_new_batch(
        self, future_indices: FutureIndices, draft_input
    ):
        intv = future_indices.interval
        ...
        if self._is_specblock():
            for name in self._SPECBLOCK_FIELDS:
                buf = getattr(self, f"{name}_buf")
                buf[intv] = getattr(draft_input, name)
            if spec_need_hidden_states():

...
class EAGLEWorker(TpModelWorker):
    ...
    def forward_batch_generation(self, batch: ScheduleBatch) -> GenerationBatchResult:
        ...
        """
        # Optional fine-grained profiling (env EAGLE_PROFILE=1).  Mirrors
        # SpecBlock-Shift's _PROFILE bucket layout for apples-to-apples
        # comparison: draft / verify_setup / verify_fwd / verify_accept /
        # refresh.  Disabled by default; the inner cuda.synchronize calls
        # inflate wall-time when on.
        import os, time
        _prof_on = os.environ.get("EAGLE_PROFILE", "0") == "1"
        if _prof_on and not hasattr(self, "_prof"):
            self._prof = {
                "draft": 0.0, "verify_setup": 0.0, "verify_fwd": 0.0,
                "verify_accept": 0.0, "refresh": 0.0, "n": 0,
                "draft_token_num_sum": 0,
            }
        ...
        if batch.forward_mode.is_extend() or batch.is_extend_in_batch:
            ...
            if _prof_on:
                ...
                self._prof["draft_token_num_sum"] += int(
            ...
            if _prof_on:
                ...
                self._prof["verify_setup"] += _t2 - _t1
            ...
            if _prof_on:
                ...
                self._prof["refresh"] += _t3 - _t2
                self._prof["n"] += 1
                if self._prof["n"] % 50 == 0:
                    p = self._prof
                    n = p["n"]
                    avg_dn = p["draft_token_num_sum"] / max(n, 1)
                    worker_hot_ms = (
                        (p["draft"] + p["verify_setup"] + p["refresh"]) / n * 1000
                    )
                    import logging
                    logging.getLogger(__name__).info(
                        f"[EAGLEprof] iter={n} avg_tree={avg_dn:.1f} | "

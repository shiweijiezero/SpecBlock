...
class SpeculativeAlgorithm(Enum):
    ...
    def is_specblock(self) -> bool:
        return self == SpeculativeAlgorithm.SPECBLOCK_SHIFT

    def supports_spec_v2(self) -> bool:
        # SpecBlock-Shift V2 wire-up under audit.
        return (
            self.is_eagle()
            or self.is_standalone()
            or self.is_specblock()
        )
    ...
    def create_worker(
        ...
        if self.is_eagle() and server_args.enable_multi_layer_eagle:
            ...
            return NGRAMWorker
        elif self.is_specblock():
            if enable_overlap:
                # V2 worker delegates to V1 internally via a
                # ModelWorkerBatch-as-ScheduleBatch shim; the overlap
                # scheduler benefit comes from BaseSpecWorker dispatch
                # at the scheduler layer, not from worker-side rewrites.
                from sglang.srt.speculative.specblock_worker_v2 import (
                    SpecBlockWorkerV2,
                )
                return SpecBlockWorkerV2

            from sglang.srt.speculative.specblock_worker import (
                SpecBlockWorker,
            )

            return SpecBlockWorker
...
class SpecInputType(IntEnum):
    ...
    MEDUSA_DRAFT = auto()
    MEDUSA_VERIFY = auto()
    SPECBLOCK_SHIFT_DRAFT = auto()
    SPECBLOCK_SHIFT_VERIFY = auto()


_DRAFT_INPUT_TYPES = frozenset(
    {
        SpecInputType.EAGLE_DRAFT,
        SpecInputType.MEDUSA_DRAFT,
        SpecInputType.SPECBLOCK_SHIFT_DRAFT,
    }
)
_VERIFY_INPUT_TYPES = frozenset(
    {
        SpecInputType.EAGLE_VERIFY,
        SpecInputType.NGRAM_VERIFY,
        SpecInputType.MEDUSA_VERIFY,
        SpecInputType.SPECBLOCK_SHIFT_VERIFY,
    }
)
...
class SpecInput(ABC):
    ...
    def is_draft_input(self) -> bool:
        ...
        return self.spec_input_type in _DRAFT_INPUT_TYPES

    def is_verify_input(self) -> bool:
        return self.spec_input_type in _VERIFY_INPUT_TYPES

...
from specforge.core.eagle3 import eval_ttt
...
def parse_args() -> Tuple[ArgumentParser, Namespace]:
    ...
    training_group.add_argument(
        ...
        type=int,
        default=None,
        help="Number of warmup steps. If provided, overrides --warmup-ratio.",
    )
    training_group.add_argument(
        "--scheduler-type",
        type=str,
        ...
        help="Learning rate scheduler type. 'linear' matches official EAGLE3.",
    )
    training_group.add_argument(
        "--total-steps",
        type=int,
...
def build_draft_model(
    ...
    Returns:
        Tuple of (draft_model_config, draft_model, resume_checkpoint_path)
        resume_checkpoint_path is None if not resuming or if using ckpt_dir for finetuning
    """
    ...
    resume_checkpoint_path = None  # Only set when resuming training (not finetuning)
...
def record_metrics(
    ...
    for i in range(len(accuracies)):
        ...
        print_on_rank0(
            f"{mode.capitalize()} - Step {global_step}, position {i}, Acc: {accuracies[i]:.2f}"
        )
    ...
    for i in range(len(plosses)):
        ...
        print_on_rank0(
            f"{mode.capitalize()} - Step {global_step}, position {i}, pLoss: {plosses[i]:.4f}"
        )
    tracker.log(logdict, step=global_step)


def record_ttt_metrics(
    args: Namespace,
    ttt_accs: List[torch.Tensor],
    ttt_accept_length: torch.Tensor,
    global_step: int,
    tracker: Tracker,
    mode: str = "eval",
) -> None:
    """Record TTT evaluation metrics."""
    logdict = {}

    # Stack and reduce ttt_accs
    ttt_accs = torch.stack(ttt_accs)
    dist.all_reduce(ttt_accs, op=dist.ReduceOp.AVG)
    ttt_accs = ttt_accs.cpu().tolist()

    for i in range(len(ttt_accs)):
        logdict[f"{mode}/ttt_acc_{i}"] = ttt_accs[i]
        print_on_rank0(
            f"{mode.capitalize()} - Step {global_step}, TTT position {i}, Acc: {ttt_accs[i]:.4f}"
        )

    # Reduce accept length (ensure it's a 1D tensor for all_reduce)
    if ttt_accept_length.dim() == 0:
        ttt_accept_length = ttt_accept_length.unsqueeze(0)
    dist.all_reduce(ttt_accept_length, op=dist.ReduceOp.AVG)
    ttt_accept_length = ttt_accept_length.cpu().item()
    logdict[f"{mode}/ttt_accept_length"] = ttt_accept_length
    print_on_rank0(
        f"{mode.capitalize()} - Step {global_step}, TTT Accept Length: {ttt_accept_length:.4f}"
    )
...
def main():
    ...
    # ================================================
    # 6. Build tracker and resume training state
    # ================================================
    tracker = build_tracker(args, parser)
    global_step = 0
    start_epoch = 0

    # Resume training state if checkpoint exists
    if resume_checkpoint_path is not None:
        ...
        if os.path.exists(training_state_path):
            ...
            # Advance scheduler to the correct step (scheduler was reset after load)
            print_on_rank0(
    ...
    # Track the step we need to resume from
    resume_global_step = global_step
    steps_per_epoch = len(train_dataloader)
    # Start current_step from the beginning of start_epoch, so it's in the same
    # reference frame as resume_global_step (which counts from training start)
    current_step = start_epoch * steps_per_epoch

    for epoch in range(start_epoch, args.num_epochs):
        # Run training
        train_dataloader.sampler.set_epoch(epoch + 1)
        eagle3_model.train()  # Use FSDP-wrapped module
        ...
        for data in progress_bar:
            ...
            # Skip steps that were already trained (during resume)
            ...
            if (
                ...
                # Run evaluation
                eagle3_model.eval()  # Use FSDP-wrapped module
                eval_acces = [[] for _ in range(eagle3_model.length)]
                eval_plosses = [[] for _ in range(eagle3_model.length)]

                # TTT evaluation metrics
                ttt_accs_all = [[] for _ in range(eagle3_model.length)]
                ttt_accept_lengths_all = []

                for eval_data in tqdm(eval_dataloader, desc=f"Evaluating Epoch {epoch}"):
                    with torch.no_grad():
                        # Standard eval (teacher forcing)
                        eval_plosses_batch, eval_acces_batch = run_forward(
                            args, eagle3_model, eval_data, target_model, is_online
                        )
                        eval_acces = [
                            eval_acces[i] + [eval_acces_batch[i]] for i in range(len(eval_acces_batch))
                        ]
                        eval_plosses = [
                            eval_plosses[i] + [eval_plosses_batch[i]] for i in range(len(eval_plosses_batch))
                        ]

                        # TTT eval (use own predictions)
                        # Use summon_full_params to ensure FSDP params are materialized
                        # when calling draft_model methods directly outside FSDP forward
                        with FSDP.summon_full_params(eagle3_model, writeback=False):
                            if is_online:
                                eagle3_data = target_model.generate_eagle3_data(
                                    input_ids=eval_data["input_ids"].cuda(),
                                    attention_mask=eval_data["attention_mask"].cuda(),
                                    loss_mask=eval_data["loss_mask"].cuda(),
                                )
                                input_ids = get_dp_data_shard_from_tp(eagle3_data.input_ids)
                                attention_mask = get_dp_data_shard_from_tp(eagle3_data.attention_mask)
                                loss_mask = get_dp_data_shard_from_tp(eagle3_data.loss_mask)
                                ...
                                attention_mask = eval_data["attention_mask"].cuda()
                                loss_mask = eval_data["loss_mask"].cuda()
                                hidden_states = eval_data["hidden_state"].cuda()

                            ttt_accs_batch, ttt_accept_length_batch = eval_ttt(
                                draft_model=eagle3_model.draft_model,
                                input_ids=input_ids,
                                attention_mask=attention_mask,
                                loss_mask=loss_mask,
                                hidden_states=hidden_states,
                                K=eagle3_model.length,
                                attention_backend=args.attention_backend,
                            )
                        ttt_accs_all = [
                            ttt_accs_all[i] + [ttt_accs_batch[i]] for i in range(len(ttt_accs_batch))
                        ]
                        ttt_accept_lengths_all.append(ttt_accept_length_batch)
                ...
                # Record TTT metrics
                ttt_accs_avg = [torch.stack(acc).mean() for acc in ttt_accs_all]
                ttt_accept_length_avg = torch.stack(ttt_accept_lengths_all).mean()

                record_ttt_metrics(
                    args,
                    ttt_accs_avg,
                    ttt_accept_length_avg,
                    global_step // args.draft_accumulation_steps,
                    tracker,
                    mode="eval",
                )
            ...
            if global_step % args.save_interval == 0:
                # Save the model
                save_checkpoints(args, epoch, global_step, eagle3_model, optimizer, vocab_mapping_path)

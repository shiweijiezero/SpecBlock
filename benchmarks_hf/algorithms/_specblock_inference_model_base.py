...
class SpecBlockAttentionWithCache(nn.Module):
    ...
    def forward(self, x, cross_cache, pos_ids, max_position: int = None,
        ...
        if _use_3way:
            from .tree_attention_triton import three_part_attention
            if _deep:
            ...
            out = three_part_attention(
                ...
                cross_count=int(cross_count) if not torch.is_tensor(cross_count) else cross_count.item(),
        ...
        if use_sdpa_single and C > 0:
            ...
            from .tree_attention_triton import single_pos_attention
            if full_kv_mask is not None:
                _kv_mask = full_kv_mask
    ...
    def forward_batch(self, x, cache, pos_ids, max_position: int, N: int,
        ...
        cache_bucket = int(os.environ.get('CACHE_COUNT_BUCKET', '0')) if use_triton else 0
        NK = N * K

        if cache_count > 0 and cache_bucket > 0 and use_triton:
            ...
            from .tree_attention_triton import tree_attention
            ...
            if use_sdpa:
                ...
                # Cross region fully visible → 0. Current region has tree mask.
                NK_v = N * K
                attn_bias = torch.zeros(NK_v, cache_count + NK_v, dtype=q.dtype, device=q.device)
                attn_bias[:, cache_count:] = batch_mask.to(q.dtype)
                out = F.scaled_dot_product_attention(
                    q, k_full, v_full,
    ...
    def forward_batch_graph_safe(self, x, cache, pos_ids, max_position: int, N: int,
                                 cross_mask=None):
        """Graph-safe batch forward.

        - No in-place cross-cache write (caller scatters + bumps count).
        - Accepts optional cross_mask for bucketed cross-cache padding.
        - Caveat: matmul over padded cache_count shape picks different
          cuBLAS/tensor-core tiling than matching eager at exact cc, which
          drifts ~3-9e-02 per forward and accumulates through layer 1 cache
          over iters. Tried fp32 attention — doesn't help (TF32 tile is also
          shape-dependent). True fix needs a shape-invariant custom attention
          kernel (triton) or forcing both eager and graph paths to the same
          padded shape.
        """
        B, NK, _ = x.shape
        K = self.K
...
class _SpecBlockInferenceModelBase(nn.Module):
    ...
    def _rank_forward(self, normed: torch.Tensor, logits: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            top_logits, _ = logits.detach().topk(_rank_topk, dim=-1)
    ...
    def forward_with_cache(self, hidden, input_ids, cache, position_id,
        ...
        pos_ids = torch.arange(
            position_id + 1, position_id + 1 + self.K, device=x.device, dtype=torch.long

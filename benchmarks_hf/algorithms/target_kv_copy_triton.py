...
@triton.jit
def _copy_selected_kv_layers_kernel(
    key_ptr,
    value_ptr,
    metadata_ptr,
    source_ptr,
    ...
    mask = valid_copy & (offsets < HEAD_DIM)
    key_base = key_ptr + layer_idx * stride_kl + row * stride_kb + head_idx * stride_kh
    value_base = value_ptr + layer_idx * stride_vl + row * stride_vb + head_idx * stride_vh
    keys = tl.load(key_base + source * stride_ks + offsets * stride_kd, mask=mask)
    values = tl.load(value_base + source * stride_vs + offsets * stride_vd, mask=mask)
    tl.store(key_base + destination * stride_ks + offsets * stride_kd, keys, mask=mask)
    tl.store(value_base + destination * stride_vs + offsets * stride_vd, values, mask=mask)


def copy_selected_kv_layers_(
    keys: torch.Tensor,
    values: torch.Tensor,
    metadata: torch.Tensor,
    sources: torch.Tensor,
) -> None:
    """Copy accepted KV for contiguous ``[L,B,H,S,D]`` storage."""
    if keys.dim() != 5 or values.shape != keys.shape:
        raise ValueError("keys and values must share [L,B,H,S,D] shape")
    ...
    _copy_selected_kv_layers_kernel[(copy_count, keys.shape[2], keys.shape[0])](
        keys,
        values,
        metadata,
        sources,

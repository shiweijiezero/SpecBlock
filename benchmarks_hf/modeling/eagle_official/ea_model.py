...
class EaModel(nn.Module):
    ...
    def eagenerate_with_timing(
        ...
        """EAGLE generate with detailed timing breakdown.

        Returns:
            input_ids: Generated token ids
            new_token: Total number of new tokens generated
            idx: Number of iterations
            timing: Dict with timing breakdown {prefill_time, target_time, verify_time, draft_time, kv_update_time}
        """
        if is_llama3:
            stop_token_id = self.tokenizer.convert_tokens_to_ids("<|eot_id|>")
        ...
        torch.cuda.synchronize()
        timing["prefill_time"] = time.perf_counter() - t0
        ...
        for idx in range(max_length):
            ...
            torch.cuda.synchronize()
            timing["verify_time"] += time.perf_counter() - t0

            # Update inputs timing (includes draft model forward)
            torch.cuda.synchronize()
            t0 = time.perf_counter()
            input_ids, draft_tokens, retrieve_indices, tree_mask, tree_position_ids, new_token, hidden_state, sample_token = update_inference_inputs(
                input_ids,
            ...
            torch.cuda.synchronize()
            timing["draft_time"] += time.perf_counter() - t0
        ...
        return input_ids, new_token, idx, timing

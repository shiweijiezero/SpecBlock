...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(
        ...
        if hasattr(self, "tree_mask") and self.tree_mask is not None:
            tree_mask = self.tree_mask  # [tree_len, tree_len]
            tree_len = tree_mask.size(-1)
            # 使用masked_fill替代boolean indexing (更稳定)
            min_val = combined_attention_mask.min()
            # 扩展tree_mask到4D
            tree_mask_4d = tree_mask.unsqueeze(0).unsqueeze(0)  # [1, 1, tree_len, tree_len]
            # 获取需要修改的slice
            mask_slice = combined_attention_mask[:, :, -tree_len:, -tree_len:]
            # 使用masked_fill: 在tree_mask==0的位置填充min_val
            combined_attention_mask[:, :, -tree_len:, -tree_len:] = mask_slice.masked_fill(
                tree_mask_4d == 0, min_val
            )

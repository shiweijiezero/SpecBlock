...
def get_last_checkpoint(folder, prefix="epoch"):
    ...
    # Match both "epoch_X" and "epoch_X_step_Y" formats
    _re_checkpoint_with_step = re.compile(r"^" + prefix + r"_(\d+)_step_(\d+)$")
    _re_checkpoint_epoch_only = re.compile(r"^" + prefix + r"_(\d+)$")

    checkpoints_with_step = []
    checkpoints_epoch_only = []

    for path in content:
        if not os.path.isdir(os.path.join(folder, path)):
            continue
        match_step = _re_checkpoint_with_step.search(path)
        match_epoch = _re_checkpoint_epoch_only.search(path)
        if match_step is not None:
            epoch, step = int(match_step.group(1)), int(match_step.group(2))
            checkpoints_with_step.append((path, epoch, step))
        elif match_epoch is not None:
            epoch = int(match_epoch.group(1))
            checkpoints_epoch_only.append((path, epoch))

    # Prefer checkpoints with step info, sorted by step number
    if checkpoints_with_step:
        best = max(checkpoints_with_step, key=lambda x: x[2])  # Sort by step
        return os.path.join(folder, best[0])
    elif checkpoints_epoch_only:
        best = max(checkpoints_epoch_only, key=lambda x: x[1])  # Sort by epoch
        return os.path.join(folder, best[0])
    return None

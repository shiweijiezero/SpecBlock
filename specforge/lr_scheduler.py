...
class LinearDecayLR(_LRScheduler):
    """Linear decay learning rate scheduler (no warmup).
    ...
    def get_lr(self):
        ...
        return [
            self.eta_min + (base_lr - self.eta_min) * (1 - self.last_epoch / self.total_steps)
            for base_lr in self.base_lrs
        ]


class LinearWarmupLR(WarmupScheduler):
    """Linear decay learning rate scheduler with learning rate warmup.

...
def replaced_logits_processor_forward_for_eagle3(
    ...
    elif logits_metadata.forward_mode.is_extend():
        # For EXTEND mode, we use the full hidden_states without pruning
        # This is specific for EAGLE3 training where we need all token logits
        pruned_states = hidden_states
        if aux_hidden_states is not None:
            aux_pruned_states = [hidden for hidden in aux_hidden_states]
        sample_indices = None
    ...
    if return_logits:
        ...
        # For EXTEND mode, we compute logits for all tokens (not just sampled ones)
